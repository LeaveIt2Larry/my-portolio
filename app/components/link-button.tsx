...
export const LinkButton = forwardRef<HTMLAnchorElement, LinkButtonProps>(function LinkButton(
  ...
  return (
    <Link
      ...
      {...rest}
      className={clsx(
        'group flex gap-3 text-3xl font-light text-dark-400 transition duration-300 ease-in-out hover:text-green-500 dark:text-dark-200',
        rest.className
      )}
    >
      {children}
      {Icon ?? (
        <VscArrowRight
          size={36}
          className='rotate-45 text-dark-200 transition duration-300 ease-in-out group-hover:translate-x-1 group-hover:translate-y-1 group-hover:text-green-300 dark:text-green-400'
        />
      )}
    </Link>
  )
